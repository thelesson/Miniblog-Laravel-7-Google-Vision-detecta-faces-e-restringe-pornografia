                    ...
                    <ul class="list-group list-group-flush">
                      <li class="list-group-item p-3">
                        <span class="d-flex mb-2">
                          ...
                          <strong class="mr-1">Status:</strong> <span id="status-publicacao">@if(isset($postEdit)) @if($postEdit->status ==="PUBLISHED") Publicado @elseif($postEdit->status ==="PENDING") Pendente @elseif($postEdit->status ==="DRAFT") Rejeitado @else Publicado @endif @else Publicado @endif</span>
                          <input class="form-control form-control-lg mb-3" id="hiddenStatus" name="status" type="hidden" value="{{ isset($postEdit) ? ($postEdit->status ? $postEdit->status : 'PUBLISHED') : 'PUBLISHED'}}">
                    
                          <a class="ml-auto" data-toggle="modal" data-target="#modalStatus" href="#">Alterar</a>
                        </span>
                        <span class="d-flex mb-2">
                          <i class="material-icons mr-1">visibility</i>
                          <input class="form-control form-control-lg mb-3" id="hiddenVisibilidade" name="visibilidade" type="hidden" value="{{ isset($postEdit) ? ($postEdit->featured ===1 ? 'logado' : 'public') : 'public'}}">
                          <strong class="mr-1">Visibilidade:</strong>
                          <strong id="txt-visibilidade" class="{{ isset($postEdit) ? ($postEdit->featured ===1 ? 'text-warning' : 'text-success') : 'text-success'}}">{{ isset($postEdit) ? ($postEdit->featured ===1 ? 'Membros' : 'Público') : 'Público'}}</strong>
                    ...
                    <strong class="mr-1">Slug:</strong><span id="textoSlug">{{ isset($postEdit) ? ($postEdit->slug ? $postEdit->slug : 'text-text') : 'text-text'}}
                      ...
                      <li class="list-group-item d-flex px-3">
                       ...
                       <button class="btn btn-sm btn-accent ml-auto" id="acoes" type="submit">
                          <i class="material-icons">save</i>@if(isset($postEdit))Atualizar  @else Criar Postagem @endif</button>
                      </li>
                    </ul>
                  </div>
                </div>
...
<div class="modal fade" id="modalStatus" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  ...
  <option  @if(isset($postEdit)) @if($postEdit->status ==="PUBLISHED") selected="selected" @endif @endif value="PUBLISHED" >Publicado </option>
  <option  @if(isset($postEdit)) @if($postEdit->status ==="DRAFT") selected="selected" @endif @endif value="DRAFT">Rejeitado </option>
  <option  @if(isset($postEdit)) @if($postEdit->status ==="PENDING") selected="selected" @endif @endif value="PENDING">Pendente</option>
</select>
      </div>
...
<div class="modal fade" id="modalVisibilidade" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  ...
  <option @if(isset($postEdit)) @if($postEdit->featured ===0) selected="selected" @endif @endif value="public" >Público </option>
  <option @if(isset($postEdit)) @if($postEdit->featured ===1) selected="selected" @endif @endif value="logado">Apenas para usuários registrados</option>
</select>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
